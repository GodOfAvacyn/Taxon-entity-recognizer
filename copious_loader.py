...
def make_spacy_datapoint(txt_path, ann_path):
    ...
    """
    text = concat_txt_file(txt_path)
    dict = create_ann_df(ann_path)
    ents = []
...
def create_dataset(path='../data/copious_published/train'):
    """
    Creates a dataset!
    """
    nlp = spacy.blank('en')
    ...
    for i in range(0, N-1, 2):
        ...
        datapoint = ()
        try:
            datapoint = make_spacy_datapoint(txt_path, ann_path)
        except:
